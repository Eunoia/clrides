...
#  lsi = Classifier::LSI.new 
 ...
 #   lsi.add_item(datum[:text],datum[:classification]) 
  ...
  miscategorized = []
  verif.map do |ver|
	debugger if train3.index(verif[0])==249+1
    #classify ver, store in resp
    #debugger if ver[:text]=~/need/
    resp = (ver[:text].split("  ")[0]=~/off/i)? :offered : :wanted
	if(resp==:wanted)
		 ver[:text].split("  ")[1..-1]=~/off/i ? :offered : :wanted 
#			print "INT"
	end
	if(resp==:wanted)
		 ver[:text].split("  ")[0]=~/back/i ? :offered : :wanted 
#			print "INT"
	end
	if(resp==:wanted)
		 ver[:text]=~/driv/i ? :offered : :wanted 
	end	
	if(resp==:wanted)
		 ver[:text].split("  ")[0]=~/avalil/i ? :offered : :wanted 
	end 
	if(resp==:wanted)
		 ver[:text].split("  ")[0]=~/\${1}\d+/ ? :offered : :wanted 
	end
